...
with filtros_panel:
    ...
    render_filter_help(
        "Cómo llenar Machine Learning",
        "El modelo predice retorno acumulado para un horizonte fijo. Volatilidad, Sharpe, VaR, beta y retorno de mercado son inputs financieros; el horizonte define cuántos meses acumula la predicción.",
    )
    c1, c2 = st.columns(2)
    with c1:
        horizon_months = st.selectbox("Horizonte fijo de predicción", [1, 3, 6, 12, 24, 36], index=3)
        model_name = st.selectbox("Modelo principal", list(MODEL_LABELS.keys()), index=2, format_func=lambda key: MODEL_LABELS[key])
...
header_dashboard(
    ...
    "Predicción de retorno acumulado a horizonte fijo con Ridge, Lasso y Gradient Boosting.",
    modo=modo,
)
...
render_info_card(
    ...
    (
        ...
        "La predicción es apoyo analítico, no recomendación automática."
    ),
)
...
if run_prediction:
    try:
        ...
        for months in [1, 3, 6, 12, 24, 36]:
            ...
            scenario_payload["horizon_months"] = months
            response = _predict(client, scenario_payload)
            sensitivity.append({"Horizonte": months, "Retorno predicho": float(response["predicted_return"])})
    except ApiClientError as exc:
...
if status:
    ...
    with c3:
        tarjeta_kpi("Horizonte", f"{horizon_months} meses", subtexto=f"Inicial: {active_horizon_label()}")

    render_meta_row(
        {
            "Tipo": status.get("model_type", "N/D"),
            "Target": status.get("target", "N/D"),
            "Singleton": "Sí" if status.get("singleton") else "N/D",
    ...
    if features:
        st.dataframe(
            pd.DataFrame(
                [
                    {"Feature": item, "Uso financiero": {
                        "volatility": "Riesgo total anualizado",
                        "sharpe_ratio": "Relación retorno-riesgo",
                        "var_95": "Pérdida extrema al 95%",
                        "beta": "Sensibilidad al benchmark",
                        "market_return": "Escenario de mercado esperado",
                        "horizon_months": "Plazo fijo de acumulación",
...
seccion("Predicción de retorno acumulado")
if prediction is not None:
    c1, c2, c3 = st.columns(3)
    with c1:
        tarjeta_kpi("Retorno acumulado", format_percent(prediction), subtexto=MODEL_LABELS[model_name])
    with c2:
        tarjeta_kpi("Volatilidad", format_percent(volatility), subtexto="Input")
    with c3:
        tarjeta_kpi("VaR 95%", format_percent(var_95), subtexto="Input")

    render_meta_row({"Sharpe": format_number(sharpe_ratio), "Beta": format_number(beta), "Mercado": format_percent(market_return)})
    nota(prediction_payload.get("interpretation", "Predicción calculada correctamente."))
    ...
    if model_predictions:
        ...
        plot_card_header("Comparación de modelos", "Ridge y Lasso aportan línea base regularizada; Gradient Boosting captura relaciones no lineales.", modo=modo)
        st.plotly_chart(
    ...
    render_info_card("Predicción pendiente", "Ejecuta el modelo para obtener retorno acumulado a horizonte fijo.")

seccion("Sensibilidad por horizonte")
if sensitivity:
    ...
    plot_card_header("Horizonte fijo", "Muestra cómo cambia el retorno acumulado cuando se modifica el plazo de predicción.", modo=modo)
    st.plotly_chart(
        style_plotly_figure(fig_h, modo=modo, title="Sensibilidad del retorno acumulado", xaxis_title="Meses", yaxis_title="Retorno acumulado"),
        use_container_width=True,
    )
    plot_card_footer("Horizontes más largos acumulan más exposición al mercado y pueden amplificar tanto retorno como pérdida esperada.")
else:
    render_info_card("Sensibilidad pendiente", "Ejecuta la predicción para construir escenarios por horizonte.")

seccion("Sustento para exposición")
render_info_card(
    "Por qué estos modelos",
    (
        "Ridge reduce inestabilidad por multicolinealidad, Lasso puede disminuir el peso de variables poco útiles y "
...
render_info_card(
    "Supuestos y límites",
    (
        "Ridge/Lasso requieren una relación aproximadamente estable entre variables y retorno. Gradient Boosting no exige linealidad, "
        "pero requiere controlar sobreajuste y revisar métricas fuera de muestra. Ningún modelo prueba causalidad ni reemplaza VaR, CAPM o stress testing."
    ),
)
